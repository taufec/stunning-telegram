  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6" onClick={onClose}>
      ...
      <motion.div 
        ...
        className="bg-[#111111] border border-white/10 rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto relative shadow-2xl z-10"
      ...
      >
        ...
        <div className="pt-16 pb-8 px-8 flex flex-col">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 pt-8 border-t border-white/10 mt-auto">
            {website_url && (
              <a href={website_url} target="_blank" rel="noopener noreferrer" className="w-full sm:w-auto flex items-center justify-center gap-2 bg-[#00FF66] text-[#111111] hover:bg-[#00CC55] px-8 py-3 rounded-full font-bold transition-colors">
                <Globe size={18} /> Visit Website
              </a>
            )}
            <div className="flex items-center gap-3 w-full sm:w-auto sm:ml-auto justify-center sm:justify-end mt-4 sm:mt-0">
              {instagram_url && (
                <a href={instagram_url} target="_blank" rel="noopener noreferrer" className="w-12 h-12 rounded-full border border-white/10 flex items-center justify-center text-white/60 hover:text-[#00FF66] hover:border-[#00FF66]/50 transition-colors bg-white/5">
                  <Instagram size={20} />
                </a>
              )}
              {tiktok_url && (
                <a href={tiktok_url} target="_blank" rel="noopener noreferrer" className="w-12 h-12 rounded-full border border-white/10 flex items-center justify-center text-white/60 hover:text-[#00FF66] hover:border-[#00FF66]/50 transition-colors bg-white/5">
                  <Music2 size={20} />
                </a>
              )}
              {linkedin_url && (
                <a href={linkedin_url} target="_blank" rel="noopener noreferrer" className="w-12 h-12 rounded-full border border-white/10 flex items-center justify-center text-white/60 hover:text-[#00FF66] hover:border-[#00FF66]/50 transition-colors bg-white/5">
                  <Linkedin size={20} />
                </a>
              )}
            </div>
